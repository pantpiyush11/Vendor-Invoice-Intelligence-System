...
def load_invoice_data():
    conn = sqlite3.connect(
        r'C:\Users\piyus\Desktop\Vendor Invoice Intelligence System\Data\inventory.db'
    )
    query = """
...
def create_invoice_risk_label(row):
    if abs(row["invoice_dollars"] - row["total_items_dollars"]) > 5:
        return 1
    if pd.notna(row["avg_recieving_delay"]) and row["avg_recieving_delay"] > 10:
        return 1
    return 0
...
def split_data(df, features, target):
    ...
    return X_train, y_train, X_test, y_test   # ✅ correct order
